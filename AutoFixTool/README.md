...
以下两项的判据较宽，**影响范围可能超出 Autodesk**，执行前请务必确认：
...
这两项保留了较宽的判据以覆盖残留场景，但值得你在使用前明确知情。
...
| 项目 | 本工具的做法 | 原因 |
| --- | --- | --- | --- |
| 目录权限授予对象 | 当前用户 / Administrators / Users | 不向 Everyone 授予完全控制，避免安全倒退 |
| 3ds Max 语言切换 | 枚举 `HKCU\\...\\Autodesk\\3dsMax` 下实际存在的版本子键 | 新版本自动覆盖，也不会写入不存在的版本 |
| hosts 修复 | 清除所有「屏蔽 Autodesk 相关域名」的条目 | 使用通用判据，不针对特定域名 |

## 已知边界

- v1.0.0 尚未在真实机器上完整验证，行为描述来自代码实现。
...
界面与代码为自有实现；修复逻辑以标准 Windows Installer / 注册表 / 服务运维方式编写。

界面布局采用通用的分类导航 + 按钮网格模式。版本历史见仓库根目录 CHANGELOG.md。
