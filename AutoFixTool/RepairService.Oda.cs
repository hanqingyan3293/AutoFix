    ...
    internal static partial class RepairService
    {
        ...
        public static string DowngradeCadFile(string filePath, int targetVersion, Action<string> log)
        {
            ...
            try
            {
                ...
                inDir = Path.Combine(Path.GetTempPath(), "AutoFix_ODAIn_" + Guid.NewGuid().ToString("N").Substring(0, 8));
                outDir = Path.Combine(Path.GetTempPath(), "AutoFix_ODAOut_" + Guid.NewGuid().ToString("N").Substring(0, 8));
