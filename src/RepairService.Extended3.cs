    ...
    internal static partial class RepairService
    {
        ...
        public static string ShrinkCadFile(string filePath, Action<string> log)
        {
            ...
            try
            {
                ...
                // 生成 AutoCAD 脚本
                scriptPath = Path.Combine(Path.GetTempPath(), "autodesk-fix_slim_" + Guid.NewGuid().ToString("N") + ".scr");
        ...
        public static string CreateRestorePoint(Action<string> log)
        {
            if (DryRun)
            {
                DryNote("创建系统还原点：Autodesk-Fix 修复前备份 " + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"), log);
                return "[预演] 将创建一个系统还原点。";
            }
            try
            {
                string desc = "Autodesk-Fix 修复前备份 " + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
