        ...
        private static void ScheduleRebootCleanup(List<string> folders, Action<string> log)
        {
            try
            {
                string logDir = Path.Combine(
                    Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "Autodesk-Fix");
                    ...
                    @"SOFTWARE\Microsoft\Windows\CurrentVersion\RunOnce", true))
                {
                    if (key != null)
                    {
                        key.SetValue("Autodesk-Fix-Cleanup", "\"" + script + "\"", RegistryValueKind.String);
        ...
        internal static string CleanOtherUserProfiles(Action<string> log)
        {
            ...
            foreach (string profile in profiles)
            {
                ...
                if (!string.IsNullOrEmpty(sid))
                {
                    ...
                    if (File.Exists(dat))
                    {
                        string tempKey = "Autodesk-FixTEMP_" + name;
                        bool mounted = false;
                        try
                        {
